...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                x= util.colToX(col)
                y= util.rowToY(row)
                dis= math.hypot(agentX- x, agentY- y)
                # p(et|ht)
                p= util.pdf(dis, Const.SONAR_STD, observedDist)
                # Update the current posterior probability with following :
                # P(Ht|E1=e1~et)= p(et|ht)* P(Ht|E1=e1~et-1)
                post= p*self.belief.getProb(row, col)
    ...
    def elapseTime(self) -> None:
        ...
        # Create a new belief to make sure we use 
        # the CURRENT self.belief distribution to compute updated beliefs
        new= util.Belief(self.belief.getNumRows(), self.belief.getNumCols(), 0)
        # Calaulate the sum of probability
        for (oldTile, newTile) in self.transProb:
            # P(Ht=ht|E1=e1~et) * p(h_t+1|ht)
            old= self.belief.getProb(oldTile[0], oldTile[1])
            tp= self.transProb[(oldTile, newTile)]
            post= old* tp
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        # Reweight the particle distribution with emission probability
        # associated with the observed distance.
        for (row, col) in self.particles:
            ...
            dis= math.hypot(agentX-x, agentY-y)
            p= util.pdf(dis, Const.SONAR_STD, observedDist)
            post= self.particles[(row, col)]* p
            self.particles[(row, col)]= post
         
        # create a new dictionary
        new= collections.defaultdict(int)
        # Re-sample the particles from the reweighted distribution
    ...
    def elapseTime(self) -> None:
        ...
        # Get the particle locations at time $t+1$
        for cur in self.particles:
            for i in range(self.particles[cur]): 
                # Choose one particle from taht locations 
                particle= util.weightedRandomChoice(self.transProbDict[cur])
                new[particle]+= 1
        self.particles= new
        # END_YOUR_CODE
