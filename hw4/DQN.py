...
class Agent():
    ...
    def learn(self):
        ...
        q_eval= self.evaluate_net(states).gather(1, actions.reshape(self.batch_size, 1))
        q_next= self.target_net(next_states).detach()
        for i in range(len(done)):
            if (done[i]): q_next[i]= 0
        q_target= rewards.reshape(self.batch_size, 1)+ self.gamma* q_next.max(1)[0].view(self.batch_size, 1)
        # Compute the loss with MSE
        func= nn.MSELoss()
        loss= func(q_eval, q_target)
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        # Use target network to estimate the maximum Q-value of the actions in the next state.
